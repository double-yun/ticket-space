...
const DUMMY_DEVICE_INFO = 'test-lottery-script'

export type LotteryTestOptions = {
  ticketCount?: number
  scenarioName?: string
  pointBalanceStrategy?: (index: number) => number
}

function generateDummyPublicKey(seed: string) {
  return Buffer.from(`test-public-key-${seed}`).toString('base64')
}

export async function runLotteryTest(userCount: number, options?: LotteryTestOptions) {
  const ticketCount = options?.ticketCount ?? userCount

  console.log(`🎲 Lottery 전체 테스트 시작${options?.scenarioName ? ` (${options.scenarioName})` : ''}\n`)
  console.log(`👥 테스트 유저: ${userCount}명`)
  console.log(`🎟️  티켓 수량: ${ticketCount}개\n`)
  ...
  for (let i = 1; i <= userCount; i++) {
    ...
    const pointBalance = options?.pointBalanceStrategy?.(i) ?? 50000
    ...
    const user = await prisma.user.create({
      data: {
        ...
        walletAddress,
        pointBalance,
        publicKey: generateDummyPublicKey(`${Date.now()}-${i}-${walletAddress}`),
        keyAlgorithm: 'ECDSA_P256',
        keyCreatedAt: new Date(),
        deviceInfo: DUMMY_DEVICE_INFO,
      },
    })
    ...
    console.log(`✅ ${String(i).padStart(2)} / ${userCount} - ${user.name} 신청 완료 (포인트: ${pointBalance})`)
  }
...
export async function cleanupLotteryTestResources() {
